...
public class GearPredictoO5 {
    ...
    public static List<Double> predict(Pair<Double, Double> position, Pair<Double, Double> vel, Pair<Double, Double> force, double dt, double m){
        ...
        for(int i = 0; i <= ORDER; i++){
            switch(i){
                case 0:
                   ...
                   break;
                case 1:
                    rp = r1 + r2*dt + r3*(Math.pow(dt, 2)/fact(2)) + r4*(Math.pow(dt, 2)/fact(2)) + r5*(Math.pow(dt, 2)/fact(2));
                    break;
                case 2:
        ...
        for(int i = 1; i <= ORDER; i++){
            double rc = predictedRs.get(i) + alpha[i]*deltaR2*fact(i)/Math.pow(dt,i); //el 2do es la velocidad corregida. Los demas no se usan creo
            correctedRs.add(rc);
        }
        return correctedRs;
    }
